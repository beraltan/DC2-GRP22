...
# Load the data
shapefile_path1 = 'data/secondary_data/map/statistical-gis-boundaries-london/ESRI/LSOA_2011_London_gen_MHW.shp'  # Original shapefile
...
# Load LSOA to Borough mapping
lsoa_borough_path = 'lsoa_new2.csv'
lsoa_borough_data = pd.read_csv(lsoa_borough_path)

# Merge the borough names into the crime data
csv_data = csv_data.merge(lsoa_borough_data, on='LSOA code', how='left')
...
def update_map(selected_shapefile, crime_types):
    ...
    crime_counts = df_filtered['Borough'].value_counts().reset_index()
    crime_counts.columns = ['Borough', 'Crime Count']
    merged_gdf = gdf.merge(crime_counts, left_on='NAME', right_on='Borough', how='left').fillna(0)
